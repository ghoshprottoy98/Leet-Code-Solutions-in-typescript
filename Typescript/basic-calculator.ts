...
class Solution {
    public calculate(s: string): number {
      ...
      for (let i = 0; i < s.length; i++) {
        if (/\d/.test(s[i])) {
          ...
          while (operators.length > 0 && precedence.hasOwnProperty(operators[operators.length - 1]) && precedence[operators[operators.length - 1]] >= precedence[s[i]]) {
            this.compute(operands, operators);
          }
          operators.push(s[i]);
        }
    ...
    private compute(operands: number[], operators: string[]): void {
